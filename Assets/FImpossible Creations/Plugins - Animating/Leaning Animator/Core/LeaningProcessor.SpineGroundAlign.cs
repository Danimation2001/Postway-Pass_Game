    ...
    public partial class LeaningProcessor
    {
        ...
        void UpdateSpineGroundAlign()
        {
            ...
            if (alignSpine)
            {
                ...
                slideLeanForw = Mathf.SmoothDamp(slideLeanForw, slopeOff * slopeFDot * effectsBlend, ref sd_slideLeanForw, SpineGrAdjustDuration, Mathf.Infinity, Time.deltaTime);

                float slopeRDot = Vector3.Dot(BaseTransform.right, slopeSlideDir);
                slideLeanSide = Mathf.SmoothDamp(slideLeanSide, slopeOff * slopeRDot * effectsBlend, ref sd_slideLeanSide, SpineGrAdjustDuration, Mathf.Infinity, Time.deltaTime);

            }
            else
            {
                slideLeanForw = Mathf.SmoothDamp(slideLeanForw, 0f, ref sd_slideLeanForw, SpineGrAdjustDuration, Mathf.Infinity, Time.deltaTime);
                slideLeanSide = Mathf.SmoothDamp(slideLeanSide, 0f, ref sd_slideLeanSide, SpineGrAdjustDuration, Mathf.Infinity, Time.deltaTime);
            }
